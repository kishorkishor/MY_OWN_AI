...
const MindMap: React.FC<MindMapProps> = ({ data: rawData, onNodeUpdate }) => {
  ...
  useEffect(() => {
    ...
    try {
        ...
        // Setup Filters (Enhanced Neumorphic Shadow)
        let defs = svg.select("defs");
        if (defs.empty()) {
            defs = svg.append("defs");
            
            // Enhanced Neumorphic shadow filter - creates raised 3D button effect
            ...
            // Outer dark shadow (bottom-right) - main depth
            ...
            // Secondary dark shadow (closer, sharper)
            neuFilter.append("feDropShadow")
                .attr("dx", "3")
                .attr("dy", "3")
                .attr("stdDeviation", "4")
            ...
            // Outer light shadow (top-left) - highlight
            neuFilter.append("feDropShadow")
                .attr("dx", "-4")
                .attr("dy", "-4")
                .attr("stdDeviation", "6")
            ...
            // Hover/Active filter - more pronounced
            const neuFilterHover = defs.append("filter")
                .attr("id", "neu-shadow-hover")
                .attr("x", "-100%")
                .attr("y", "-100%")
                .attr("width", "300%")
                .attr("height", "300%");
            
            neuFilterHover.append("feDropShadow")
                .attr("dx", "8")
                .attr("dy", "8")
        ...
        // NEUMORPHIC SHOCKWAVE (Subtle ripples in grey spectrum)
        const rippleCount = 2;
        for (let i = 0; i < rippleCount; i++) {
            nodeEnter.append("circle")
                ...
                .duration(800 + (i * 200))
                .ease(d3.easeQuadOut)
                .attr("r", (d: any) => calcRadius(d.weight) * (3 + i))
                .attr("opacity", 0)
                .remove();
        }

        // PARTICLE SPARKLES (6 particles - grey spectrum)
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            nodeEnter.append("circle")
                ...
                .duration(600)
                .ease(d3.easeQuadOut)
                .attr("cx", (d: any) => Math.cos(angle) * calcRadius(d.weight) * 2)
                .attr("cy", (d: any) => Math.sin(angle) * calcRadius(d.weight) * 2)
                .attr("r", 0)
                .attr("opacity", 0)
                .remove();
        }

        // Main Circle - ENHANCED NEUMORPHIC BUBBLE with pop animation
        ...
        nodeEnter.append("circle")
            ...
            .transition()
            .duration(400)
            .ease(d3.easeBackOut.overshoot(1.5)) // Bouncy pop effect
            .attr("r", (d: any) => calcRadius(d.weight))
            .attr("opacity", 1)
            .transition()
            .duration(200)
            .ease(d3.easeQuadInOut)
            .attr("r", (d: any) => calcRadius(d.weight) * 0.95) // Slight settle
            .transition()
            .duration(150)
            .ease(d3.easeQuadOut)
            .attr("r", (d: any) => calcRadius(d.weight)); // Final size
