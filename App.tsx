...
const App: React.FC = () => {
  ...
  // Auto-Update Mind Map Effect - Enhanced for real-time updates with pop animations
  useEffect(() => {
    // Trigger when model message completes streaming
    if (messages.length > 0) {
        const lastMsg = messages[messages.length - 1];
        if (lastMsg.role === 'model' && !lastMsg.isStreaming && !isChatLoading) {
            // Update when streaming completes - animations will pop new nodes
            const timer = setTimeout(() => {
                updateMindMapBackground();
            }, 300); // Small delay to ensure message is fully processed
  ...
  const updateMindMapBackground = async () => {
    if (isBackgroundUpdating) return; // Prevent overlapping updates
    setIsBackgroundUpdating(true);
    try {
        // Use safeGenerate but also explicitly catch here to be safe
        const mapData = await safeGenerate(generateMindMapData(messages));
        if (mapData) {
            // Direct update - D3 will detect new nodes and animate them with pop effect
            setMindMapData(mapData);
        }
    } catch (e) {
        console.warn("Auto-update of mind map failed", e);
    } finally {
  ...
  // Quick action handler - sends a prompt directly to chat
